...
#### Этап 3: Заполнение пустот (опционально)
...
#### Этап 6: Разбиение на осколки (ShardGenerator) — Grid-based Voronoi
...
- **Алгоритм (Grid-based Voronoi):**
  1. Получает список клеток острова из `SimpleIslandTracer` (через `IslandData.Cells`)
  2. Генерирует N случайных точек-сайтов внутри острова (в grid-координатах):
     - `N = area / 100` (1 точка на 100 кв.единиц), max 20
     - Проверяет что клетка solid через HashSet
     - Seed для воспроизводимости: `baseSeed + chunkX * 1000 + chunkY`
  3. Назначает каждую solid клетку ближайшему сайту (по евклидову расстоянию)
  4. Для каждой группы клеток извлекает контур через edge tracing (`SimpleIslandTracer.ExtractContourFromSet`)
- **Результат:** Осколки тайлят остров идеально (без щелей и наложений), как медовые соты
- **Выход:** `List<List<Vector2>>` - список осколков в локальных координатах chunk'а

**Почему Grid-based, а не polygon clipping:**
- Sutherland-Hodgman НЕ работает с вогнутыми clip-полигонами (island contours вогнутые)
- S-H вычисляет пересечение полуплоскостей → kernel полигона → пусто для вогнутых форм
- Grid-based подход гарантирует идеальный тайлинг без артефактов

#### Этап 7: Создание физических тел (PhysicsBodyBuilder)
- **Класс:** `PhysicsBodyBuilder.CreateBodyFromShard(shard)`
- **Вход:** Осколок (может быть вогнутым после grid-based Voronoi)
- **Алгоритм:**
...
- Подходит для вогнутых полигонов (центроид может быть снаружи, но вырожденные треугольники фильтруются)
